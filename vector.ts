...
export async function index_directory(dirPath: string) {
    const ext = await getExtractor();
    
    let files: string[] = [];
    function walk(dir: string) {
        if (files.length > 50) return; // Limit for performance
        try {
            ...
            for (let file of list) {
                ...
                else if (stat.isFile() && stat.size < 100000) files.push(fp);
            }
        } catch (e) {}
    }
    walk(dirPath);
    ...
    for (const file of files) {
        try {
            ...
            await new Promise((resolve) => {
                db.run("INSERT INTO vector_index (filepath, content, vector) VALUES (?, ?, ?)", 
                    [file, content, JSON.stringify(vectorArray)], resolve);
            });
            count++;
        } catch (e) {}
    }
...
export async function semantic_search(query: string): Promise<string> {
    const ext = await getExtractor();
    const output = await ext(query, { pooling: 'mean', normalize: true });
    const queryVector = Array.from(output.data) as number[];

    return new Promise((resolve) => {
        db.all("SELECT filepath, content, vector FROM vector_index", (err, rows: any[]) => {
