...
def get_model_name(m):
    return getattr(m, 'model', None) or getattr(m, 'name', None) or (m.get('model') if isinstance(m, dict) else None) or (m.get('name') if isinstance(m, dict) else str(m))
...
def print_models_table(models_list, title="Available Local Models"):
    ...
    panel = Panel(Align.center(table), title=f"[bold green]{title}[/bold green]", border_style="cyan")
    console.print()
    console.print(panel)
    console.print()
...
[bold yellow]/export <file>[/bold yellow]   - Export the chat to a beautiful HTML file
    """
    console.print(Panel(help_text, title="[bold cyan]💡 Keyboard & Command Shortcuts[/bold cyan]", border_style="cyan", expand=False))
...
def handle_slash_command(command, args, model_name, messages, config):
    ...
    if command == "/clear":
        if Confirm.ask("[bold red]Are you sure you want to clear the conversation history?[/bold red]"):
            clear_history(current_session)
            messages.clear()
            console.print(Panel("[bold green]🗑️ Conversation history cleared![/bold green]", border_style="green", expand=False))
        return True, model_name
        
    elif command == "/models":
        try:
            available_models = ollama.list()
            models_list = getattr(available_models, 'models', []) if hasattr(available_models, 'models') else available_models.get('models', [])
        ...
        if args:
            ...
            console.print(Panel(f"[bold green]🔄 Switched active model to:[/bold green] [bold yellow]{new_model}[/bold yellow]", border_style="green", expand=False))
        ...
        if args:
            ...
            try:
                ...
                console.print(Panel(f"[bold green]💾 ✅ Conversation successfully saved to:[/bold green] {filename}", border_style="green", expand=False))
            except Exception as e:
        ...
        try:
            ...
            console.print(Panel(f"[bold green]🌐 ✅ Exported chat to HTML:[/bold green] {filename}", border_style="green", expand=False))
        except ImportError:
        ...
        if Confirm.ask("[bold red]⚠️ Execute this code on your machine?[/bold red]"):
            try:
                ...
                if Confirm.ask("Send output back to the assistant?"):
                    handle_turn(f"I ran the code. Here is the output:\n```\n{out}\n```", messages, model_name, config)
            except Exception as e:
                print_error(str(e))
        return True, model_name
        ...
        try:
            ...
            handle_turn(prompt, messages, model_name, config, display_input="Generate git commit message for current changes")
        ...
        with console.status(f"[bold yellow]🎨 Generating image for '{query}'...[/bold yellow]", spinner="dots"):
            import requests
            ...
            try:
                ...
                with open(filename, 'wb') as f:
                    f.write(r.content)
                console.print(Panel(f"[bold green]✅ Image successfully generated and saved to:[/bold green] {filename}", border_style="green", expand=False))
            except Exception as e:
        ...
        try:
            ...
            for conv in data:
                ...
                imported += 1
            console.print(f"[bold green]✅ Imported {imported} conversations from ChatGPT![/bold green]")
        except Exception as e:
                    ...
                    if p.is_file() and not p.name.startswith('.') and 'node_modules' not in p.parts:
                        try:
                            ...
                            for i, chunk in enumerate(chunks):
                                ...
                                collection.add(documents=[chunk], metadatas=[{"source": str(p)}], ids=[doc_id])
                            count += 1
                        except:
                            pass
                console.print(Panel(f"[bold green]✅ Ingested {count} files into the local Vector Database![/bold green]", border_style="green", expand=False))
            except Exception as e:
                print_error(str(e))
        return True, model_name
        ...
        with console.status("[bold yellow]🔍 Searching local documents...[/bold yellow]"):
            try:
                ...
                console.print(Panel(context, title="[cyan]Retrieved Context[/cyan]", border_style="cyan"))
        ...
        agent_messages = [
            {"role": "system", "content": "You are a terminal execution agent. YOU HAVE REAL TERMINAL ACCESS. Your ONLY purpose is to output executable code blocks. Do not act like an AI model. Do not apologize.\nTo execute a command, output a ```bash\n<command>\n``` block. \nWhen the task is fully complete, output exactly 'DONE' by itself on the last line.\n\nEXAMPLE:\nUser: Task: find my username\nAssistant:\n```bash\nwhoami\n```\nUser: Execution Output:\n```\nvishal\n```\nAssistant:\nThe username is vishal.\nDONE"},
            {"role": "user", "content": f"Task: {task}"}
        ]
        
        console.print(Panel(f"[bold magenta]🤖 Autonomous Agent Loop Started:[/bold magenta] {task}\n[dim]The agent will think and execute iteratively.[/dim]", border_style="magenta"))
        ...
        while iteration < max_iterations:
            ...
            try:
                ...
                console.print(Panel(content, title="[cyan]Agent Thought[/cyan]", border_style="cyan"))
                ...
                if not blocks:
                    console.print("[yellow]No executable code found. Asking agent to proceed...[/yellow]")
                    agent_messages.append({"role": "user", "content": "CRITICAL INSTRUCTION: You failed to provide a code block. DO NOT apologize. DO NOT act like an AI. You MUST provide a ```bash``` or ```python``` block to execute, or output 'DONE' if the task is finished."})
                    continue
                    
                lang, code = blocks[-1]
                console.print(Panel(code, title=f"[yellow]Executing {lang}[/yellow]", border_style="yellow"))
                ...
                console.print(Panel(out[:1000] + ("..." if len(out)>1000 else ""), title="[green]Execution Output[/green]", border_style="green"))
                
                agent_messages.append({"role": "user", "content": f"Execution Output:\n```\n{out}\n```\nWhat is the next step? (Output a code block or DONE)"})
        ...
        if iteration >= max_iterations:
            console.print("[bold red]❌ Agent reached maximum iterations (10) and was forcefully stopped.[/bold red]")
...
[dim]System Prompts: {sys_chars} characters[/dim]
        ...
        handle_turn(prompt, messages, model_name, config, display_input="<Pasted Clipboard Content>")
        return True, model_name
        ...
        try:
            if command == "/format":
                ...
                console.print(Panel(f"```python\n{formatted_code}\n```", title="[bold green]Formatted Code (Black)[/bold green]", border_style="green"))
                ...
                if result.stdout:
                    console.print(Panel(result.stdout, title="[bold red]Lint Errors found by flake8[/bold red]", border_style="red"))
                    if Confirm.ask("Ask assistant to fix these errors?"):
                        ...
                        handle_turn(prompt, messages, model_name, config, display_input="Fix linting errors")
                else:
                    console.print(Panel("[bold green]No linting errors found! Perfect code.[/bold green]", border_style="green", expand=False))
        except Exception as e:
...
def main():
    ...
    parser.add_argument("-w", "--workspace", type=str, default="default", help="Workspace session name")
    ...
    if not model_name:
        try:
            ...
            if not models_list:
                print_error("No local models found. Please pull a model first (e.g. 'ollama pull llama3')")
                sys.exit(1)
            ...
            if len(models_list) == 1:
                ...
                choice = IntPrompt.ask("[bold cyan]Enter the number of the model to use[/bold cyan]", choices=[str(i+1) for i in range(len(models_list))], show_choices=False)
    ...
    slash_commands = ['/search', '/model', '/models', '/pull', '/copy', '/paste', '/clear', '/save', '/export', '/help', '/run', '/commit', '/new', '/chats', '/load', '/sysinfo', '/stats', '/format', '/lint', '/logs', '/imagine', '/agent', '/agents', '/ingest', '/ask', '/cd', '/ls', '/pwd', '/import_openai', 'exit', 'quit']
    ...
    if file_context:
        ...
        if path.is_dir():
            ...
            console.print(Panel(tree, title="[bold cyan]Attached Directory Structure[/bold cyan]", border_style="cyan"))
            
            # Read files for LLM context (basic implementation)
            text_content = ""
            try:
                for p in path.rglob("*"):
                    if p.is_file() and not p.name.startswith('.') and 'node_modules' not in p.parts and '__pycache__' not in p.parts:
                        try:
                            with open(p, 'r') as f:
                                text_content += f"\n--- {p} ---\n{f.read()[:5000]}\n" # limit per file
                        except: pass
    ...
    while True:
        try:
            ...
            if user_input.startswith('/'):
                ...
                if cmd == "/search":
                    with console.status("[bold yellow]🌐 Searching the web...[/bold yellow]", spinner="dots"):
                        results = search_web(cmd_args)
                    preview = results[:300] + "..." if len(results) > 300 else results
                    console.print(Panel(preview, title="[bold yellow]🔍 Search Results[/bold yellow]", border_style="yellow"))
                    augmented_prompt = f"Please answer the user's query based on these web search results:\n\n{results}\n\nUser Query: {cmd_args}"
                    handle_turn(augmented_prompt, messages, current_model_name, config, display_input=cmd_args)
                    console.print(Rule(style="dim cyan"))
                    continue
                        ...
                        try:
                            for response in ollama.pull(cmd_args, stream=True):
                                if 'total' in response and 'completed' in response:
                                    progress.update(task, completed=response['completed'], total=response['total'])
                                elif 'status' in response:
                                    progress.update(task, description=f"[cyan]{response['status']}")
                            console.print(f"[bold green]✅ Successfully pulled {cmd_args}![/bold green]")
                        except Exception as e:
                    ...
                    if last_assistant_response:
                        ...
                        console.print("[bold green]📋 ✅ Copied last assistant response to clipboard![/bold green]")
                    else:
                    ...
                    handled, current_model_name = handle_slash_command(cmd, cmd_args, current_model_name, messages, config)
                    if handled:
                        console.print(Rule(style="dim cyan"))
                        continue
    ...
    console.print(Panel("[bold green]Goodbye! Keep building awesome things.[/bold green]", border_style="green", expand=False))
